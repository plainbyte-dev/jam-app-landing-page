...
export default function HeroSection() {
  ...
  const spot1Ref      = useRef<SVGGElement>(null);
  const spot2Ref      = useRef<SVGGElement>(null);
  const spot3Ref      = useRef<SVGGElement>(null);
  const streakRef     = useRef<SVGPathElement>(null);

  useEffect(() => {
    if (!headlineRef.current || !statsRef.current) return;
    ...
    // ── Floating ball ────────────────────────────────────────────────────────
    gsap.to(".hero-ball", {
      y: -16, duration: 2.2, ease: "sine.inOut", yoyo: true, repeat: -1,
    });

    // ── Pulse rings ──────────────────────────────────────────────────────────
    gsap.to(".hero-ring", {
      scale: 1.08, opacity: 0.4, duration: 2, ease: "sine.inOut",
      yoyo: true, repeat: -1, stagger: 0.4,
    });

    // ── Scan line ────────────────────────────────────────────────────────────
    gsap.to(".scan-line", {
      y: "100%", duration: 2.5, ease: "none", repeat: -1,
    });

    // ── Spotlight sway ───────────────────────────────────────────────────────
    if (spot1Ref.current) {
      gsap.to(spot1Ref.current, {
        rotation: 6, transformOrigin: "50% 0%",
        duration: 4, ease: "sine.inOut", yoyo: true, repeat: -1,
      });
    }
    if (spot2Ref.current) {
      gsap.to(spot2Ref.current, {
        rotation: -7, transformOrigin: "50% 0%",
        duration: 5.5, ease: "sine.inOut", yoyo: true, repeat: -1,
      });
    }
    if (spot3Ref.current) {
      gsap.to(spot3Ref.current, {
        rotation: 5, transformOrigin: "50% 0%",
        duration: 3.8, ease: "sine.inOut", yoyo: true, repeat: -1,
      });
    }

    // ── Ball trajectory streak dash-offset ──────────────────────────────────
    if (streakRef.current) {
      const len = (streakRef.current as SVGPathElement).getTotalLength?.() ?? 400;
      gsap.set(streakRef.current, { strokeDasharray: len, strokeDashoffset: len });
      gsap.to(streakRef.current, {
        strokeDashoffset: 0, duration: 2, ease: "power2.inOut",
        delay: 1.8, yoyo: true, repeat: -1, repeatDelay: 1.2,
      });
    }
  }, []);
  ...
  return (
    <section
      ...
      style={{ background: "linear-gradient(135deg, #040c06 0%, #060e08 50%, #080f09 100%)" }}
    >
      {/* ── BACKGROUND LAYER ────────────────────────────────────────────── */}
      <ArenaBackground spot1Ref={spot1Ref} spot2Ref={spot2Ref} spot3Ref={spot3Ref} streakRef={streakRef} />
        ...
        {PARTICLES.map((p) => (
          <span
            ...
            style={{
              ...
              background: "#00E676",
              opacity: p.opacity,
              animation: `particleFloat ${p.duration} ${p.delay} ease-in-out infinite alternate`,
            }}
          />
        ))}
      </div>
      ...
      {/* ── PITCH LINES OVERLAY ─────────────────────────────────────────── */}
      <div className="pitch-lines absolute inset-0 pointer-events-none" />
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 60% 70% at 70% 50%, rgba(0,230,118,0.07) 0%, transparent 70%)",
        }}
      />
      ...
      <div className="absolute bottom-0 right-0 w-48 h-48 pointer-events-none opacity-20 rotate-180">
        <svg viewBox="0 0 200 200" fill="none">
          <path d="M0 60 L0 0 L60 0" stroke="#00E676" strokeWidth="1" />
        </svg>
      </div>
      ...
      <style>{`
        @keyframes particleFloat {
          ...
          50%  { transform: translateY(-22px) scale(1.4); opacity: calc(var(--op, 0.2) * 1.6); }
          100% { transform: translateY(-8px) scale(0.8); opacity: calc(var(--op, 0.2) * 0.5); }
        }
      `}</style>
    </section>
  );
...
interface ArenaBackgroundProps {
  spot1Ref: RefObject<SVGGElement | null>;
  spot2Ref: RefObject<SVGGElement | null>;
  spot3Ref: RefObject<SVGGElement | null>;
  streakRef: RefObject<SVGPathElement | null>;
}

function ArenaBackground({ spot1Ref, spot2Ref, spot3Ref, streakRef }: ArenaBackgroundProps) {
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      <svg
      ...
      >
        <defs>
          {/* Spotlight gradients */}
          <radialGradient id="spotGrad1" cx="50%" cy="0%" r="100%" fx="50%" fy="0%">
            <stop offset="0%"   stopColor="#00E676" stopOpacity="0.18" />
            <stop offset="60%"  stopColor="#00E676" stopOpacity="0.04" />
            <stop offset="100%" stopColor="#00E676" stopOpacity="0" />
          </radialGradient>
          <radialGradient id="spotGrad2" cx="50%" cy="0%" r="100%" fx="50%" fy="0%">
            <stop offset="0%"   stopColor="#00E676" stopOpacity="0.12" />
            <stop offset="60%"  stopColor="#00E676" stopOpacity="0.03" />
            <stop offset="100%" stopColor="#00E676" stopOpacity="0" />
          </radialGradient>
          <radialGradient id="spotGrad3" cx="50%" cy="0%" r="100%" fx="50%" fy="0%">
            <stop offset="0%"   stopColor="#a8ff78" stopOpacity="0.10" />
          ...
          <linearGradient id="floorGrad" x1="0" y1="0" x2="0" y2="1">
            ...
            <stop offset="60%"  stopColor="#00E676" stopOpacity="0.03" />
            <stop offset="100%" stopColor="#00E676" stopOpacity="0.07" />
          </linearGradient>
        ...
        {/* Floor base */}
        <path
          d="M0 900 L1440 900 L1100 520 L340 520 Z"
          fill="url(#floorGrad)"
          opacity="0.6"
        />
        ...
        {[0, 1, 2, 3, 4, 5].map((i) => {
          const y = 520 + i * 64;
          const shrink = i / 5;
          const x1 = 340 - shrink * 340;
          const x2 = 1100 + shrink * 340;
          return (
            <line
              key={`h${i}`}
        ...
        {/* Floor perspective lines — vertical (converging to vanishing point ~720,460) */}
        {[-4, -3, -2, -1, 0, 1, 2, 3, 4].map((i) => {
          const vx = 720;
          const vy = 460;
          const baseX = 720 + i * 130;
          return (
            <line
              key={`v${i}`}
              x1={vx} y1={vy}
        ...
        <ellipse cx="720" cy="720" rx="120" ry="38" stroke="#00E676" strokeWidth="0.8" opacity="0.08" />
        <ellipse cx="720" cy="720" rx="60"  ry="19" stroke="#00E676" strokeWidth="0.6" opacity="0.06" />

        {/* ── Ceiling / arena top truss lines ─────────────────────────── */}
        {[200, 400, 600, 800, 1040, 1240].map((x, i) => (
          <line
            key={`truss${i}`}
        ...
        {/* Ceiling cross-beams */}
        <line x1="0" y1="60" x2="1440" y2="60" stroke="#00E676" strokeWidth="0.4" opacity="0.04" />
        <line x1="0" y1="120" x2="1440" y2="120" stroke="#00E676" strokeWidth="0.3" opacity="0.03" />

        {/* ── Spotlight 1 — left ───────────────────────────────────────── */}
        <g ref={spot1Ref as React.RefObject<SVGGElement>}>
          {/* Lamp fixture */}
          <rect x="215" y="0" width="20" height="8" rx="2" fill="#00E676" opacity="0.4" />
          <rect x="222" y="8" width="6" height="14" fill="#00E676" opacity="0.3" />
          {/* Beam cone */}
          <path
            d="M225 22 L130 620 L320 620 Z"
            fill="url(#spotGrad1)"
            opacity="0.9"
          />
          {/* Inner bright core */}
          <path
            d="M225 22 L190 420 L260 420 Z"
            fill="#00E676"
            opacity="0.06"
          />
          {/* Light circle on floor */}
          <ellipse cx="225" cy="620" rx="95" ry="22" fill="#00E676" opacity="0.04" filter="url(#softGlow)" />
          {/* Dust motes in beam */}
          {[80, 150, 220, 310, 400].map((y, j) => (
            <circle
              key={`d1${j}`}
              cx={225 + (j % 2 === 0 ? j * 4 : -j * 3)}
              cy={y}
              r="1.2"
              fill="#00E676"
              opacity={0.25 - j * 0.04}
        ...
        {/* ── Spotlight 2 — center-right ───────────────────────────────── */}
        <g ref={spot2Ref as React.RefObject<SVGGElement>}>
          <rect x="830" y="0" width="20" height="8" rx="2" fill="#00E676" opacity="0.3" />
          <rect x="837" y="8" width="6" height="14" fill="#00E676" opacity="0.25" />
          <path
            d="M840 22 L720 540 L960 540 Z"
            fill="url(#spotGrad2)"
            opacity="0.7"
          />
          <path
            d="M840 22 L790 380 L890 380 Z"
            fill="#00E676"
            opacity="0.04"
          />
          <ellipse cx="840" cy="540" rx="120" ry="26" fill="#00E676" opacity="0.03" filter="url(#softGlow)" />
          {[70, 160, 250, 340].map((y, j) => (
            <circle
              key={`d2${j}`}
              cx={840 + (j % 2 === 0 ? -j * 5 : j * 4)}
              cy={y}
              r="1"
              fill="#00E676"
              opacity={0.2 - j * 0.04}
        ...
        {/* ── Spotlight 3 — far right accent ───────────────────────────── */}
        <g ref={spot3Ref as React.RefObject<SVGGElement>}>
          <rect x="1295" y="0" width="16" height="7" rx="2" fill="#a8ff78" opacity="0.25" />
          <rect x="1301" y="7" width="4" height="12" fill="#a8ff78" opacity="0.2" />
          <path
            d="M1303 19 L1220 500 L1386 500 Z"
            fill="url(#spotGrad3)"
            opacity="0.6"
          />
          <ellipse cx="1303" cy="500" rx="83" ry="18" fill="#a8ff78" opacity="0.025" filter="url(#softGlow)" />
        </g>

        {/* ── Ball trajectory arc streak ───────────────────────────────── */}
        <path
          ref={streakRef as React.RefObject<SVGPathElement>}
          d="M 350 680 Q 580 320 850 420 Q 990 470 1100 560"
        ...
        <circle cx="1100" cy="560" r="7" fill="#ffffff" opacity="0.12" filter="url(#bgGlow)" />

        {/* ── Background silhouettes — crowd / stands ──────────────────── */}
        <g opacity="0.035">
          {/* Simple crowd silhouettes as bumpy row */}
          <path
            d="M0 840 Q30 820 60 838 Q90 820 120 836 Q150 818 180 834 Q210 816 240 832
               Q270 814 300 830 Q330 812 360 828 Q390 810 420 826 Q450 808 480 824
               Q510 806 540 822 Q570 804 600 820 Q630 802 660 818 Q690 800 720 816
               Q750 798 780 814 Q810 796 840 812 Q870 794 900 810 Q930 792 960 808
               Q990 790 1020 806 Q1050 788 1080 804 Q1110 786 1140 802 Q1170 784 1200 800
               Q1230 782 1260 798 Q1290 780 1320 796 Q1350 778 1380 794 Q1410 776 1440 792
               L1440 900 L0 900 Z"
            fill="#00E676"
          />
        </g>

        {/* ── Bleacher light strip ──────────────────────────────────────── */}
        <line x1="0" y1="820" x2="1440" y2="820" stroke="#00E676" strokeWidth="0.6" opacity="0.04" />
        ...
        {/* ── Side ambient glow pools ───────────────────────────────────── */}
        <ellipse cx="100"  cy="500" rx="180" ry="60" fill="#00E676" opacity="0.025" filter="url(#softGlow)" />
        <ellipse cx="1340" cy="480" rx="160" ry="55" fill="#00E676" opacity="0.02"  filter="url(#softGlow)" />
      </svg>
    </div>
  );
}

// ─── Futsal Court Illustration ─────────────────────────────────────────────────
function FutsalCourtIllustration() {
  return (
    <div className="relative w-full max-w-lg">
      ...
      >
        <div className="text-xs mb-0.5" style={{ color: "#5a8060" }}>Next slot</div>
